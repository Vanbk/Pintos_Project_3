		...
		uint32_t read_bytes, uint32_t zero_bytes, bool writable)
{
	...
	while (read_bytes > 0 || zero_bytes > 0)
	{
		...
		bid = -1;
		// a little bit modification for project 4 => do not care about that
		if (!writable)
		{
			inode = file_get_inode(file);
			if (inode != NULL)
			/* Returns the block device sector that contains byte offset load_offset
 			within INODE.
 			Returns -1 if INODE does not contain data for a byte at load_offset
			POS. */
