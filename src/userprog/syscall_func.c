...
// search for in formation of the file in the list by file descriptor
fd_to_file(int fid)
{
...
//Maps the file fd into address addr
...
mapid_t system_call_mmap(int fd, void *address)
{
	...
	fs = fd_to_file(fd); // get the file structure of the file that owns fd
	if (fs != NULL)
	f = file_reopen(fs->f);
	else
	{
	...
	//reject the mmap when Zero address, Zero file length, Address is not page-aligned, Addr is in code or stack page
