		...
		off_t block_id)
{
	...
	if (type == TYPE_ZERO)
	{
		...
		{
			p->type = TYPE_ZERO;
			// doing something to find a new page (find free frame)
			// pagedir_op_page(uint32_t *pd, void *uaddr, void *vm_page)
			// vm_page != null -> perform add
			// perform look_page with CREATE attribute is tru 
...
// The eviction process simply ignores all pinned frame. A simple solution was to pin the frame while it is being read in
bool _pin(bool operation, void *pagetemp, bool directFrameAccess)
{
	...
	{
		struct frame_struct *f = _address_to_frame(address);
		if (f != NULL)
		{
			switch (operation)
			{
...
bool _operation_page(int operation, void *address, void * kpage, bool pinned)
{
	...
	if (operation == OP_LOAD)
	{
		...
		struct frame_struct *vf = _address_to_frame(page->physical_address);
		if (vf == NULL)
			return false;
...
struct page_struct *_find_page(void *address)
{
	...
	if (pagedir != NULL)
	{
		...
		// pagedir_op_page return void pointer -> have to convert to page_struct 
